 ...
 */
const matchScript = require ('./run-script-os-custom-functions.js').matchScript;
const expandShorthand = require('./run-script-os-custom-functions.js').expandShorthand;
...
if (!process.env["npm_config_argv"]) {
  if(process.argv.length > 1){
    process.env["npm_config_argv"] = `{"remain":[],"cooked":["run","${process.argv[2]}"],"original":["run","${process.argv[2]}"]}`
    console.log(`npm_config_argv manually set using ${process.argv[2]}`)
  }
  else{
